...
export function isValidEventPlacement(
  ...
  const consumptionPeriods = getConsumptionPeriods(
    balance,
    otherEvents,
    eventValues,
  );
...
export function findValidPlacement(
  ...
  for (
    let time = minTime;
    time <= maxTime - (event.consumes.includes(balance) ? event.duration : 0);
    time += step
  ) {
    if (
  ...
  for (const balance of balances) {
    for (const event of events) {
      ...
      if (
        ...
        conflicts.push(
          `${event.name} conflicts on ${balance} at time ${eventStart}`,
        );
      }
    }
  }
...
export function findValidPlacementAllBalances(
  ...
  for (let time = minTime; time <= maxTime - maxDuration; time += step) {
    if (
      isValidEventPlacementAllBalances(
        eventName,
        time,
        event,
        events,
